  ...
  const copyToClipboard = async (text: string, field: string) => {
    try {
      ...
      toast({
        ...
        description: t('toast.fieldCopied', { field }),
      });
    } catch (error) {
      toast({
    ...
    <Card className="mb-6 border-dashed border-2 border-orange-200 bg-orange-50">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg text-orange-800">{t('auth.testMode.title')}</CardTitle>
          {isMockMode && <Badge variant="secondary" className="bg-orange-100 text-orange-800">{t('auth.testMode.active')}</Badge>}
        </div>
        <CardDescription className="text-orange-700">
          {t('auth.testMode.description')}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {!isMockMode ? (
          ...
          >
            ...
            {t('auth.testMode.enableButton')}
          </Button>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-2">
                <label className="text-sm font-medium text-orange-800">{t('auth.testMode.testEmail')}</label>
                <div className="flex items-center gap-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-orange-800">{t('auth.testMode.testPassword')}</label>
                <div className="flex items-center gap-2">
                  <code className="flex-1 px-2 py-1 bg-white rounded border text-sm">
