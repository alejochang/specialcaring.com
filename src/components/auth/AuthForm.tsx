...
const AuthForm = ({ type }: AuthFormProps) => {
  ...
  const handleSocialSignIn = async (provider: "google" | "twitter" | "facebook") => {
    if (isMockMode) {
      toast({
        ...
        description: "Social login is not available in test mode. Use the test credentials instead.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-2xl shadow-sm border border-border">
      {type === "login" && (
        <MockAuthToggle
          onUseMockAuth={enableMockMode}
          onUseRealAuth={disableMockMode}
      ...
      {/* Social Login Buttons - disabled in mock mode */}
      {!isMockMode && (
        <>
