...
import { PlusCircle, Pencil, Trash2, Plus, Phone, MapPin, AtSign, Star, Loader2 } from "lucide-react";
...
const MedicalContacts = () => {
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      setContacts((data || []) as any as DbContact[]);
    } catch (error: any) {
            ...
            <Card key={contact.id} className="hover:shadow-md transition-shadow overflow-hidden bg-white">
              <CardHeader className="relative pb-2">
                <div className="absolute top-4 right-4 flex gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDelete(contact.id)} className="h-8 w-8"><Trash2 size={14} /></Button>
                </div>
