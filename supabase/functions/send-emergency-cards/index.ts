...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch data using the export function's logic
    const exportResponse = await fetch(
      "https://ogkieklnxxmvjgikyzog.supabase.co/functions/v1/export-emergency-cards",
    ...
    const emailResult = await resend.emails.send({
      ...
      to: [recipientEmail],
      subject: `Emergency Information Card for ${patientName}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: #7C3AED; color: white; padding: 20px; border-radius: 8px 8px 0 0;">
            <h1 style="margin: 0;">🆘 Emergency Information</h1>
            <p style="margin: 8px 0 0 0; opacity: 0.9;">Shared via Special Caring</p>
          </div>
          <div style="padding: 20px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
            <p>Hi${recipientName ? ` ${recipientName}` : ""},</p>
            <p>Please find the emergency information card for <strong>${patientName}</strong> below.</p>
            <p style="color: #6b7280; font-size: 14px;">Keep this information accessible for emergencies.</p>
    ...
    console.log("Emergency card email sent:", emailResult);

    return new Response(JSON.stringify({ success: true, data: emailResult }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error: any) {
    console.error("Error sending emergency card email:", error);
